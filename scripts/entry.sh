...
# Make sure the steam directory exists
mkdir -p "${STEAMAPPDIR}"
...
if [ -n "${CACHEDIR}" ]; then
  echo "Setting cachedir to ${CACHEDIR}"
...
if [ -n "${MODFOLDERS}" ]; then
  echo "Setting modfolders to ${MODFOLDERS}"
...
if [ -n "${ADMINPASSWORD}" ]; then
  echo "Setting admin password to ***"
...
if [ -n "${SERVERNAME}" ]; then
  echo "Setting servername to ${SERVERNAME}"
  ARGS="${ARGS} -servername ${SERVERNAME}"
else
  echo "Server name is not set ${SERVERNAME}, defaulting to servertest"
